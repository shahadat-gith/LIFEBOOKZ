...
export default function ProfilePage() {
 ...
 async function handleSubmit(e) {
  ...
  try {
   if (avatarFile) {
    const fd = new FormData(); fd.append('fullName', fullName); fd.append('avatar', avatarFile);
    await updateUser(fd);
   } else await updateUser({ fullName });
 ...
 return <div className="max-w-2xl mx-auto py-10 px-4">
  <div className="flex items-center gap-6 mb-10">
   <div className="relative group">
    <Avatar src={user.avatar?.url} name={user.fullName} size="xl" className="ring-4 ring-border" />
   ...
   <div><h1 className="text-2xl font-bold">{user.fullName}</h1><p className="text-sm text-muted-foreground">{user.email}</p><p className="text-xs text-muted-foreground mt-1">Member since {new Date(user.createdAt).toLocaleDateString()}</p>{avatarFile && <p className="text-xs text-primary mt-1">Selected: {avatarFile.name}</p>}</div>
  </div>
  <Card className="mb-6">
   <form onSubmit={handleSubmit}>
    <CardContent className="p-6"><CardTitle className="mb-4">Edit Profile</CardTitle>
     <div className="space-y-4"><Input label="Name" value={fullName} onChange={e => setFullName(e.target.value)} required /><Input label="Email" value={user.email} disabled helperText="Email cannot be changed" /></div>
    </CardContent>
