...
interface Testimonial {
  ...
  content: string;
  badge: string;
  badgeVariant: 'success' | 'primary' | 'info';
  gradient: string;
}
          ...
          {testimonials.map((testimonial) => (
            ...
            >
              <div className="relative h-full p-6 sm:p-8 rounded-2xl bg-card border border-border/50 hover:border-border transition-all duration-300 hover:shadow-lg">
                {/* Gradient accent */}
                <div
                  className={`absolute top-0 left-0 right-0 h-1 rounded-t-2xl bg-gradient-to-r ${testimonial.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
                />
