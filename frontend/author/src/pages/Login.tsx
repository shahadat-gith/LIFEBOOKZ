      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        <Card className="p-8 backdrop-blur-xl bg-card/90 border-border/50 shadow-2xl">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-muted-foreground mt-1">Sign in to manage your stories</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {error && <div className="text-sm text-destructive flex items-center gap-1.5 p-3 rounded-lg bg-destructive/10"><Icons.exclamationCircle className="h-4 w-4 flex-shrink-0" />{error}</div>}
            <Button type="submit" fullWidth size="lg" loading={loading} icon={<Icons.login className="h-4 w-4" />}>Sign In</Button>
          </form>

          <div className="mt-6 text-center">
            <p className="text-sm text-muted-foreground">Not registered? <Link to="/register" className="text-primary font-semibold hover:text-primary/80 transition-colors">Apply to become an author</Link></p>
          </div>
        </Card>
      </motion.div>
    </div>
