...
export function Card({ children, hover = false, padding = 'md', className = '', ...props }) {
  return (
    <div
      className={`bg-card text-card-foreground rounded-xl border border-border shadow-xs ${hover ? 'hover:shadow-md hover:border-muted-foreground/30 transition-all duration-200 cursor-pointer' : ''} ${paddingClasses[padding]} ${className}`}
      {...props}
    >
      {children}
    </div>
