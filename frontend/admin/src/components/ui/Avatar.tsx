...
export default function Avatar({ src, name, size = 'md', className = '' }: P) {
  ...
  return <div className={\`rounded-full flex items-center justify-center font-semibold text-white \${sc[size]} \${className}\`} style={{ background: color(name) }}>{initials(name)}</div>;
}
